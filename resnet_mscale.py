...
class ResNet(nn.Module):
    def __init__(self, in_shape, in_dims, num_blocks, num_classes):
        ...
        self.flex_pool1 = nn.Parameter(torch.ones(16, in_shape, in_shape) / in_shape ** 2)
        self.linear1 = nn.Linear(16, num_classes)

        self.layer2 = self._make_layer(32, num_blocks[1], stride=2)
        layer2_shape = ceil(in_shape / 2)
        self.flex_pool2 = nn.Parameter(torch.ones(32, layer2_shape, layer2_shape) / layer2_shape ** 2)
    ...
    def forward(self, x):
        ...
        if self.training:
            fp1 = self.flex_pool1.view(16, -1).softmax(1).view(self.flex_pool1.shape)  # FlexPool weights
            out1 = (out * fp1).sum((2, 3))  # (BS, 16)

        out = self.layer2(out)  # (BS, 32, 16, 16)
        if self.training:
            fp2 = self.flex_pool2.view(32, -1).softmax(1).view(self.flex_pool2.shape)  # FlexPool weights
            out2 = (out * fp2).sum((2, 3))  # (BS, 32)

        out = self.layer3(out)  # (BS, 64, 8, 8)  (last feature map)
        fp3 = self.flex_pool3.view(64, -1).softmax(1).view(self.flex_pool3.shape)  # FlexPool weights
        out = (out * fp3).sum((2, 3))  # (BS, 64)
        if self.training:
            return self.linear1(out1), self.linear2(out2), self.linear3(out)
        return self.linear3(out)


def resnet20(inshape, in_dims=3, num_classes=10):
    return ResNet(inshape, 
                  in_dims, [3, 3, 3], num_classes)  # 3 stages, with 3 res-blocks per stage
